...
UPSTREAM_GIT="../bitlbee-facebook/.git"
...
sed -i -e "0,/${OUR_HASH}/s//${THEIR_HASH}/1" bitlbee-facebook.spec
sed -i -e "s/%changelog/%changelog\\
* ${logdate} ${COMMITTER}\\
- update to ${THEIR_HASH}\\
/" bitlbee-facebook.spec

cp bitlbee-facebook.spec "${SPECDIR}"

# create tarball of upstream git
git --git-dir="${UPSTREAM_GIT}" archive --prefix="bitlbee-facebook-${THEIR_HASH}/" --output="${SOURCEDIR}/bitlbee-facebook-${shorthash}.tar.gz" HEAD
